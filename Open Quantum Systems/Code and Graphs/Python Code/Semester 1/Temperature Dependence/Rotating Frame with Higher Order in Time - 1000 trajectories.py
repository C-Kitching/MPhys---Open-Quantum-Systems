...
number_of_traj = 10 # number of trajectories

# record populations and times for each trajectory
master_pop = []
master_time = []
...
for i in range(number_of_traj):
    ...
    master_pop.append(pop)
    master_time.append(time)
    
# calculate average pop
avg_pop = np.mean(master_pop, axis = 0)

# calculate error of average dist
std = np.std(master_time, axis = 0) / np.sqrt(number_of_traj)

# plot errorbar plot
plt.errorbar(master_time, avg_pop, yerr = std, capsize = 2.5, errorevery = len(master_time)//20, ecolor = 'm',
