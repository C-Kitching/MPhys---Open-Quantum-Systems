...
number_of_traj = np.arange(200,1001,200)

master_counts = [] # track counts for each time

# loop over all times
for k in range(len(number_of_traj)):
    
    counts = [] # store counts of each trajectory
    
    for i in range(number_of_traj[k]):
...
for l in range(len(master_counts)):
    ...
    # plot histogram
    _, bin_borders, _ =plt.hist(master_counts[l], bins = 20, align = 'mid', rwidth = 0.9, density = True, alpha = 0.3, 
             label = 'traj = %s, p_value = %.2f' %(number_of_traj[l], p_value))
...
plt.legend()
plt.title('Quantum Jump Occurance for different trajectory number with t = %s, dt = %s' %(tmax, dt))
plt.show()
