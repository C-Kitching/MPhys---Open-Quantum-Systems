...
if __name__ == '__main__':
    ...
    # different regimes
    regime = [0.02*pow(10, i) for i in range(7)]
    ...
    # spin states
    plt.figure()
    plt.plot(norm_regime, master_up, label = r'$\rho_{\uparrow}$')
    plt.plot(norm_regime, master_down, label = r'$\rho_{\downarrow}$')
    plt.xlabel(r'$t_{max}$')
    ...
    # energy states
    plt.figure()
    plt.plot(norm_regime, master_plus, label = r'$\rho_{++}$')
    plt.plot(norm_regime, master_minus, label = r'$\rho_{--}$')
    plt.xlabel(r'$log(t_{max})$')
    plt.ylabel('$\overline{P}$')
    plt.legend()
    plt.title('Final energy-state population with ramp speed')
    
    # heat and work
    plt.figure()
    plt.plot(norm_regime, master_work, label = r'Work')
    plt.plot(norm_regime, master_heat, label = r'Heat')
    plt.xlabel(r'$log(t_{max})$')
    plt.ylabel('Energy scale')
    plt.legend()
    plt.title('Heat transferred and work done with ramp speed')
