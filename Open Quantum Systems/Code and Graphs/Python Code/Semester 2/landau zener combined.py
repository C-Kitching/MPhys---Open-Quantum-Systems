...
for i in range(len(tmax_LZ)):
    ...
    for i in range(number_of_traj):
        ...
        temp_ground, temp_excited, temp_time = single_MC(initial_state, t)
...
# plot results
plt.subplot(1,2,2)
plt.plot(np.log10(bias_rate_of_change), master_pop, label = r'$\rho_{++}$')
plt.plot(np.log10(bias_rate_of_change), LZ_prob, label = r'$P_{A}$', linestyle = '--')
plt.xlabel('$log\\left(\\frac{d\epsilon}{dt}\\right)$', fontsize = 14)
plt.ylabel('Probability', fontsize = 14)
plt.legend()
plt.title('Adiabatic basis', fontsize = 14)
...
# plot results
plt.subplot(1,2,1)
plt.plot(np.log10(bias_rate_of_change), master_pop, label = r'$\rho_{\downarrow\hspace{-0.5}\downarrow}$')
plt.plot(np.log10(bias_rate_of_change), LZ_prob, label = r'$P_{D}$', linestyle= '--')
plt.xlabel('$log\\left(\\frac{d\epsilon}{dt}\\right)$', fontsize = 14)
plt.ylabel('Probability', fontsize = 14)
plt.legend()
plt.title('Diabatic basis', fontsize = 14)


# common title
plt.suptitle('Landau-Zener vs Quantum trajectories', fontsize = 16)
plt.tight_layout()
